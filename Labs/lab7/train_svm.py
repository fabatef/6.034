...
def train_svm(training_points, kernel_fn=dot_product, max_iter=500,
                ...
                if show_update:
                    if manual_animation:
                        ...
                        if animation_delay > 0:
                            ...
                            last_update_time = time()
                        update_svm_plot(svm)
            print("Boundary drawn/updated. Number of support vectors: {}".format(len(svm.support_vectors)), svm.support_vectors)
    ...
    print("Training complete! SVM with decision boundary %.3f*x + %.3f*y + %.3f >= 0 misclassified %i points." \
        % (svm.w[0], svm.w[1], svm.b, len(misclassified)), margin_width(svm))
